...
SELECT name FROM actor JOIN casting ON actorid = actor.id JOIN movie ON movie.id = movieid WHERE title = 'Alien'
...
(SELECT id FROM movie WHERE title IN (SELECT title FROM movie JOIN casting ON movie.id = movieid WHERE actorid IN ( SELECT ID FROM actor WHERE name = 'Art Garfunkel')))
