...
FROM (SELECT DISTINCT r1.num, r1.company, s2.name FROM route r1 JOIN route r2 ON (r1.company=r2.company AND r1.num=r2.num) 

JOIN stops s1 ON (r1.stop=s1.id AND s1.name='Craiglockhart') 

JOIN stops s2 ON (s2.id=r2.stop)) AS bus1
 
 JOIN (SELECT DISTINCT r1.num, r1.company, s2.name FROM route r1 JOIN  route r2 ON (r1.company=r2.company AND r1.num=r2.num) 
 
 JOIN stops s1 ON (r1.stop=s1.id AND s1.name='Lochend') 
 
 JOIN stops s2 ON (s2.id=r2.stop)) AS bus2 
 
 ON bus1.name=bus2.name ORDER BY bus1.num, bus1.company, bus1.name, bus2.num, bus2.company;